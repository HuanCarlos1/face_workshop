...
MyImage::MyImage ( const char * fileName, const char * format ):QImage(fileName,format){
    pt=NULL;u1=NULL;u2=NULL;u3=NULL;
}
...
void MyImage::init(){
    ...
    /*			std::cout<<"Totalbits*4 "<<TotalBits*4<<"\n\n";
                        for(i=0;i<TotalBits*4;i++){
                        std::cout<<(int)pt[i]<<" ";
                        if(i%10==0)std::cout<<"\n";
                        }
...
MyImage MyImage::fusionTwoImages(const MyImage & img1, const MyImage &img2, double r){
    QImage qi;
    qi = img1.copy();
    int i,j;
    QColor q1, q2;
    for (i=0; i<qi.width(); i++)
        for (j=0; j<qi.height(); j++){
            q1 = img1.pixel(i, j);
            q2 = img2.pixel(i, j);
            qi.setPixel(i, j, (QColor(q1.red()*r+q2.red()*(1-r),
